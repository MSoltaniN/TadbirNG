...
import { BrowserStorageService, SessionKeys } from "@sppc/shared/services";
import { lastValueFrom } from "rxjs";
import { MetaDataColumns } from "@sppc/shared/models/MetaDataColumns";
 ...
 */
  ...
  ngOnInit() {
    this.settingService.updateGridColumns.subscribe(res => {
      if (res == 'update') {
        this.entityTypeName = this.entityName;
      }
    })
  }
  ...
  ngOnDestroy() {
    ...
    let currentColumns: MetaDataColumns[] = this.getMetaData(this.entityName)? this.getMetaData(this.entityName).columns: '';
    let currentSetting = this.settingService.getSettingByViewId(this.viewId);

    if (currentColumns && currentSetting) {
      let columnViews = currentSetting.columnViews;

      currentColumns.forEach(col => {
        let isExistCol = columnViews.find(i => i.name == col.name);

        if (!isExistCol && col.visibility != "AlwaysHidden")
          columnViews.push(JSON.parse(col.settings));

      });
    }

    if (currentSetting)
      lastValueFrom(this.settingService.putUserSettings(this.UserId, currentSetting))
      .then(response => {
  ...
  public getMetaData(name: string): any {
    let metadataKey = String.Format(
      SessionKeys.MetadataKey,
      this.viewId ? this.viewId.toString() : "",
      this.CurrentLanguage
    );
    var viewId = ViewName[name];

    if (viewId) {
      let item = this.bStorageService.getMetadata(metadataKey);
  ...
  setSettingGridRow() {
    ...
    this.allGridColumn.forEach(item => {
      ...
      if (columnView.visibility != ColumnVisibility.AlwaysHidden) {
        ...
        rowItem.title = item.groupName ? columnView.title + String.Format(" ({0})", item.groupName) : columnView.title;
