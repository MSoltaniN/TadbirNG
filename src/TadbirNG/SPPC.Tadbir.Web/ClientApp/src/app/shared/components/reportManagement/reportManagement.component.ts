  ...
  implements OnInit
{
  ...
  @ViewChild(ReportParametersComponent, {static: false})
  public reportParameter: ReportParametersComponent;
  @ViewChild(TreeViewComponent, {static: false}) public treeView: TreeViewComponent;
  @ViewChild(TabsComponent, {static: false}) public tabsComponent: TabsComponent;
  ...
  onNodeClick(e: any) {
    ...
    this.qReport = false;
    if (data.isDynamic) this.qReport = true;

    if (!data.isGroup) {
      if (data.id > -100) this.currentReportId = data.id;
      this.currentReportName = data.caption;
      this.deleteConfirmMsg = String.Format(
        this.getText("Report.DeleteReportConfirm"),
        data.caption
      );
  ...
  onNodeDblClick(dataItem: any) {
    ...
    if (!data.isGroup) {
      ...
      this.disEnAllButtons(false);
      this.showReport();
    } else this.disEnAllButtons(true);
  }

  //متد نمایش گزارش فوری
  public showQuickReport(
    viewId: string,
    formParams: Array<ReportParamComponent>,
    filter: FilterExpression = null,
    sort: SortDescriptor[] = null,
    designJson: string,
    qrRowData: any,
    quickReportViewInfo: QuickReportConfigInfo,
    quickFilter: FilterExpression = null
  ) {
  ...
  public showDialog(
    viewId: string = null,
    formParams: Array<ReportParamComponent> = null,
    filter: FilterExpression = null,
    sort: SortDescriptor[] = null,
    treeMenuData: any = null,
    ...
    if (viewId) {
      ...
      this.reportingService.getAll(url).subscribe((res: any) => {
   ...
   * @param formParams
   */
  public addQReportToDefaultFolder(
    viewId: string,
    formParams: Array<ReportParamComponent>
  ) {
    var expandKeysArray: string[];

    var defaltReportUrl = String.Format(
      ReportApi.ReportsByViewQuickReportUrl,
      viewId
    );
    if (!this.qReport)
      defaltReportUrl = String.Format(ReportApi.ReportsByViewDefault, viewId);

    this.reportingService.getAll(defaltReportUrl).subscribe((res: any) => {
      ...
      while (nodeData.parentId != null) {
        ...
        var parentNode = this.treeData.filter(
          (p: any) => p.id == nodeData.parentId
        );
  ...
  public showReport() {
    if (
      this.currentDefaultReportId == this.currentReportId ||
      this.currentViewId
    ) {
  ...
  public setDefaultForAll() {
    var defaltReportUrl = String.Format(
      ReportApi.ReportDefault,
      this.currentReportId
    );
    this.reportingService.setDefaultForAll(defaltReportUrl).subscribe(
      (res: any) => {},
      (error) => {},
      () => {
  ...
  private initViewer() {
    if (this.CurrentLanguage == "fa")
      Stimulsoft.Base.Localization.StiLocalization.setLocalizationFile(
        "assets/reports/localization/fa.xml"
      );

    if (this.CurrentLanguage == "en")
      Stimulsoft.Base.Localization.StiLocalization.setLocalizationFile(
        "assets/reports/localization/en.xml"
      );
  ...
  public createFilters(params: ParameterInfo[], cfilter?: FilterExpression) {
    ...
    if (params) {
      params
        .filter((p) => p.controlType.toLowerCase() != "querystring")
    ...
    if (cfilter) {
      if (
        filters.findIndex(
          (f) =>
            f.FieldName.toLowerCase() == cfilter.filter.FieldName.toLowerCase()
        ) == -1
      )
      ...
      cfilter.children.forEach(function (f) {
        if (
          filters.findIndex(
            (f) => f.FieldName.toLowerCase() == f.FieldName.toLowerCase()
          ) == -1
        )
  ...
  okSaveAsClick() {
    ...
    this.reportingService.saveAsReport(url, localReport).subscribe(
      (response: any) => {
  ...
  public prepareReport(formParams: Array<ReportParamComponent>) {
    ...
    this.reportingService.getAll(url).subscribe((res: Response) => {
      ...
      if (printInfo.parameters.length > 0) {
        ...
        if (
          formPrameters.length == 0 ||
          formPrameters.findIndex((p) => p.value == null) > -1
        )
  ...
  public previewReport(params: ParameterInfo[] = null) {
    ...
    if (
      ...
      this.currentDefaultReportId == this.currentPrintInfo.id
    ) {
      serviceUrl = this.DefaultServiceUrl;
      urlIsComplete = true;
    } else serviceUrl += this.currentPrintInfo.serviceUrl;
    ...
    if (!urlIsComplete) {
      ...
      if (params && params.length > 0) {
        ...
        params
          .filter((p) => p.source.toLowerCase() == "route")
          .forEach(function (p) {
    ...
    if (!this.Parameters || this.Parameters.length == 0) {
      this.reportingService
        ...
        .subscribe((response: any) => {
          ...
          if (this.qReport) reportTemplate = this.quickReportJsonDesign;
          else reportTemplate = this.currentPrintInfo.template;
      ...
      this.reportingService
        ...
        .subscribe((response: any) => {
          ...
          else if (response.body.items) data = response.body.items;
          else data = data = response.body;
  ...
  updateDateColumnByMetadata() {
    this.metadataKey = String.Format(
      ...
      this.currentQuickReportViewInfo.viewId
        ? this.currentQuickReportViewInfo.viewId.toString()
        : "",
      this.currentlang
    );
    ...
    columns
      .filter(
        (c) =>
          c.dotNetType.toLowerCase() == "system.date" ||
          c.dotNetType.toLowerCase() == "system.datetime"
      )
      .forEach((col) => {
        var index = this.currentQuickReportViewInfo.columns.findIndex(
  ...
  replaceServiceUrlParams(url: string, params: ReportParamComponent[]): string {
    if (params.length > 0) {
      ...
      params.forEach((item) => {
        if (item.ParamValue) args.push(item.ParamValue);
      });

      if (args.length > 0) url = String.Format(url, args);
    }
  ...
  changeServiceUrl(url: string, params: ParameterInfo[]): string {
    var queryStringParams = params.filter(
      (p) => p.source.toLowerCase() === "querystring"
    );
    if (queryStringParams.length > 0) url += "?";
    else return url;
    var itemCount = 0;
  ...
  saveDesignOfReport(id: string) {
    var designer = new Stimulsoft.Designer.StiDesigner(
      null,
      "StiDesigner" + id.replace("designerTab", ""),
      false
    );
  ...
  updateTemplateInTab(designer: any) {
    var tab = this.tabsComponent.dynamicTabs.find(
      (t) => t.Id == "designerTab" + this.currentReportId
    );
    if (tab) {
  ...
  designReport() {
    ...
    var tabIsOpen = this.tabsComponent.openTab(
      this.currentReportName,
      null,
      null,
      true,
    ...
    designer.onSaveReport = function (e: any) {
      ...
      service.saveReport(url, localReport).subscribe(
        (response: any) => {
  ...
  deleteReport(deleteFlag: boolean) {
    ...
    if (deleteFlag) {
      ...
      this.reportingService.deleteReport(url).subscribe(
        (response: any) => {
  ...
  public setClass(dataItem: any): any {
    var cssClass = "";
    if (dataItem.isGroup) cssClass = "rep-folder";
    if (!dataItem.isGroup && dataItem.isSystem) cssClass = "rep-system";
    if (!dataItem.isGroup && !dataItem.isSystem) cssClass = "rep-user";

    if (dataItem.id == this.currentDefaultReportId) cssClass += " def";
  ...
  public directShowReport() {
    ...
    var jsonString = this.bStorageService.getQuickReportSetting(
      viewId,
      userId.toString()
    );
    if (jsonString) {
  ...
  public showDefaultReport(viewInfo: QuickReportConfigInfo = null) {
    ...
    this.reportingService.getAll(url).subscribe((res: any) => {
      ...
      if (showQReport) {
        defaultReport = treeData.filter(
          (t: any) => t.isDynamic === true && t.isDefault === true
        )[0];
      }
  ...
  public showDefaultDesignedReport() {
    ...
    this.reportingService.getAll(url).subscribe((res: any) => {
      treeData = <Array<TreeItem>>res.body;
      var defaultReport = treeData.filter(
        (t: any) => t.isDefault === true && t.isDynamic === false
      )[0];

      treeData = treeData.filter(
        (t: any) => t.id !== this.quickReportFolderId && t.isDynamic === false
      );
    ...
    if (showQReport) {
      ...
      this.reportingService.getAll(url).subscribe((res: Response) => {
        ...
        if (printInfo.parameters.length > 0) {
          ...
          params.forEach(function (p) {
            if (p.ParamReportVisible == false || !p.ParamInFilter) {
              var index = reportParameters.findIndex(
                (f) => f.name === p.ParamName
              );
              if (index >= 0) reportParameters.splice(index, 1);
            }
          });
        ...
        var url = String.Format(
          ReportApi.EnvironmentQuickReport,
          viewInfo.inchValue
        );

        this.reportingService
          .putEnvironmentUserQuickReport(url, viewInfo)
          .subscribe((response: any) => {
