...
export class LoginComponent extends DefaultComponent implements OnInit {
  ...
  constructor(
    ...
    @Host() public parent: LoginContainerComponent,
  ...
  changeLang(language: string) {
    ...
    if (this.currentlang == "fa") {
      ...
      if (
        this.document.getElementById("sppcFont").getAttribute("href") !=
        "assets/resources/IranSans.css"
      )
        this.document
          .getElementById("sppcFont")
          .setAttribute("href", "assets/resources/IranSans.css");
    }
    ...
    if (this.currentlang == "en") {
      ...
      if (
        this.document.getElementById("sppcFont").getAttribute("href") !=
        "assets/resources/IranSans-en.css"
      )
        this.document
          .getElementById("sppcFont")
          .setAttribute("href", "assets/resources/IranSans-en.css");
    }
  }
  ...
  disableLink(fileName: string) {
    ...
    for (var i = 0; i < links.length; i++) {
      ...
      if (
        ...
        //link.disabled = true;
        if (link.getAttribute("href") === fileName) link.disabled = true;
      }
    }
  }
  ...
  login() {
    ...
    this.authenticationService
      ...
      .subscribe(
        ...
        (error) => {
          this.toastrService.error(this.getText("Login.UserIncorrect"), "", {
  ...
  checkInternetConnection() {
    if (!navigator.onLine) {
      this.showMessage(
        this.getText("Messages.InternetConnectionError"),
        MessageType.Info
      );
      this.logOut();
      return false;
  ...
  startActivatingSoftware() {
    this.licenseService
      .ActivateLicense(LicenseApi.ActivateLicenseUrl)
      .subscribe(
        (res) => {
          this.closeActivationForm();
          this.showMessageWithTime(
            this.getText("Messages.ActivationIsSuccessful"),
            MessageType.Succes,
            4000
          );
          this.bStorageService.setContext(
        ...
        (error) => {
          if (error.type == ErrorType.InvalidUserPass) {
            ...
            this.showMessageWithTime(
              this.getText("Messages.ActivationIsNotSuccessful"),
              MessageType.Error,
              4000
            );
          }
  ...
  startCheckingOnlineLicense() {
    ...
    this.licenseService
      .CheckOnlineLicense(
        String.Format(LicenseApi.OnlineUserLicenseUrl, this.currentUserId)
      )
      .subscribe(
        (res) => {
          this.showMessageWithTime(
            this.getText("Messages.OnlineLicenseIsSuccessful"),
            MessageType.Succes,
            4000
          );
        ...
        (error) => {
          if (error.type == ErrorType.InvalidUserPass) {
            ...
            this.showMessageWithTime(
              this.getText("Messages.OnlineLicenseIsNotSuccessful"),
              MessageType.Error,
              4000
            );
          }
  ...
  startCheckLicense() {
    ...
    if (this.duringCheckOnlineLicense) {
      this.startCheckingOnlineLicense();
    }
  ...
  tooManySessionsMessage() {
    this.showMessage(
      this.getText("Messages.TooManySessions"),
      MessageType.Info
    );
    this.logOut();
    ...
    this.licenseService
      .GetOpenSessions(
        String.Format(LicenseApi.OpenSessionsByUserUrl, this.currentUserId)
      )
      .subscribe((res) => {
  ...
  removeSession(id: number) {
    ...
    this.licenseService
      .DeleteOpenSessions(
        String.Format(LicenseApi.OpenSessionsUrl),
        modelsIdArray
      )
      .subscribe((res) => {
        this.sessions.splice(
          this.sessions.findIndex((f) => f.id == id),
          1
        );
