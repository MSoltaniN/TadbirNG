...
export class ErrorListComponent extends AutoGeneratedClientGridComponent implements OnInit {
  ...
  isColumnValuesNull(item) {
    // propName to camelCase
    let propName = (<String>item.name).replace(item.name.charAt(0),item.name.charAt(0).toLowerCase());
    return !(this.rowData.filter(i => i[propName]).length > 0);
  }
