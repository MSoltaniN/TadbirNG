...
import { SppcFullAccountDetailComponent } from "@sppc/shared/controls/fullAccount/sppc-full-account-detail/sppc-full-account-detail.component";
...
{  
  ...
  async getFunctions() {
    let res = await this.widgetService.getFunctions().pipe(
      map(res => res as WidgetFunction[])
    ).toPromise()
  ...
  async getTypes() {
    let res = await this.widgetService.getTypes().pipe(
      map(res => res as WidgetType[])
    ).toPromise()
  ...
  removeHandler() {
    ...
    this.modelUrl = DashboardApi.Widget;
