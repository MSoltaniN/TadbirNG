...
export class DetailComponent extends BaseComponent implements OnDestroy {
  ...
  private fillFormValidators() {
    ...
    if (this.getProperties(this.metadataKey)) {
      for (let entry of this.getProperties(this.metadataKey)) {
        ...
        if (entry.length > 0)
          validators.push(Validators.maxLength(entry.length));
    ...
    if (this.properties[this.metadataKey] == undefined) {
      this.metadataService
        ...
        .subscribe((res1: any) => {
          ...
          this.bStorageService.setMetadata(
            this.metadataKey,
            this.properties[this.metadataKey]
          );
  ...
  isFormChanged() {
    ...
    if (model) {
      ...
      return (
        JSON.stringify(modelCopy1) != JSON.stringify(modelCopy2) ||
        (this.insertedInNew && self.isNew ? true : false)
      );
    } else {
      return false;
    }
  ...
  async saveChangesConfirmDialog(cb?: {
    onSave?: Function;
    onDiscard?: Function;
  }) {
    let result;
    ...
    if (result?.mode == 1) {
      if (cb?.onSave) cb.onSave();
      else this.saveChangesHandler();

      return false;
    } else {
      if (cb?.onDiscard) cb.onDiscard();
      else this.discardChangesHandler();
