...
import { CalendarType, Entities, MessageType } from "@sppc/shared/enum/metadata";
import {
  ColumnViewConfig,
  Error,
  ListFormViewConfig,
  ...
  implements OnDestroy
{
  ...
  getRowValue(columnName: string, dataItem: any, dataType: string): any {
    ...
    switch (dataType.toLowerCase()) {
      ...
      case "boolean": {
        if (res == true) {
          result = this.getText("Account.Active");
        } else {
          result = this.getText("Account.Inactive");
        }
        break;
      }
      case "currency":
  ...
  public getExportData(operationId:number = OperationId.Export): Observable<GridDataResult> {
        ...
        row.cells.forEach((cell) => {
          ...
          if (this.gridColumns[cellNo]) {
            if (this.CurrentLanguage == "fa") {
              ...
              if (
                this.gridColumns[cellNo].storageType.toLowerCase() == "time" &&
                cell.value
              ) {
                cell.value = cell.value.toString().toPersianNumbers(cell.value);
              }
              ...
              if (
                this.gridColumns[cellNo].storageType.toLowerCase() == "datetime" &&
                cell.value
              ) {
                let formt;
                if (this.gridColumns[cellNo].scriptType.toLowerCase() == "datetime")
                  formt = "YYYY/MM/DD HH:mm";
                else
                  formt = "YYYY/MM/DD";
  ...
  refetchGridColumns() {
    ...
    this.showloadingMessage = false;
    this.gridColumns = properties.filter((p) => !(<string>p.visibility)?.includes('Hidden'));
  }
  ...
  // > Next Added
  highLightNewRow(item:any,customClass?:string) {
    setTimeout(() => {
        ...
        if (!isNew) {
          ...
          service.insert<T>(serviceUrl, model).subscribe(
            (response: any) => {
              ...
              if (this.entityTypeName == Entities.Currency) {
                this.highLightNewRow(model.code);
              } else {
                this.highLightNewRow(model);
              }
