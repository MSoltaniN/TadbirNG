...
import { ViewName, TestBalancePermissions, AccountPermissions } from '@sppc/shared/security';
...
import { AccountInfo, TestBalanceService, VoucherService } from '@sppc/finance/service';
...
import { BookDisplayType, BranchScopeType, VoucherStatusType } from '@sppc/finance/enum/shared';
...
@LoadPersist()
export class TestBalanceComponent extends AutoGeneratedGridComponent implements OnInit {
  ...
  @Persist<boolean>() useClosingVoucher: boolean = false; 
  @Persist<boolean>() useClosingTempVoucher: boolean = false;
  @Persist<boolean>() showZeroBalanceItems: boolean = false;
  ...
  @Persist<boolean>() disableAccountLookup: boolean = true;
  
  showFilterByRef: boolean = false;

  @Persist<string>() formatSelected: string = BalanceFormatType.Balance6Column; 
  
  @Persist<number>() displayTypeSelected: number = 0;  
  @Persist<string>() branchScopeSelected: string = BranchScopeType.CurrentBranch;  
  @Persist<string>() voucherStatusSelected: string = VoucherStatusType.Committed;  
  @Persist<boolean>() selectedBranchSeparation: boolean = false;
  ...
  @Persist<string[]>() selectedReferences: string[];
  ...
  constructor(public toastrService: ToastrService, public translate: TranslateService, public dialogService: DialogService, public gridService: GridService,
    ...
    , public testBalanceService: TestBalanceService, public lookupService: LookupService, public voucherService: VoucherService, public router: Router,public shareDataService:ShareDataService) {
  ...
  onChangeFilterByRef(event) {
    if (!this.isAccess(Entities.TestBalance, TestBalancePermissions.FilterByRef)) {
      setTimeout(() => {
  ...
  showAccountBook() {
    ...
    if (this.testBalanceType == '1') {
      ...
      if (this.clickedRowItem) {
        ...
        account.name = this.clickedRowItem.accountName;
        account.id = this.clickedRowItem.accountId;
        account.level = this.breadCrumbList[this.breadCrumbList.length - 1].displayType.level - 1;
  ...
  getReportData(accountId?: any, clearbreadCrumb: boolean = true) {
    ...
    if (this.voucherStatusSelected == "0") {
      this.showMessage(this.getText("Balance.AllDocsSelectHint"), MessageType.Info);
    }
  ...
  onDataBind(resData: any) {
    ...
    if (resData.items.length == 0 && this.clickedRowItem) {
      this.navigateToBalanceByAccount();
    }
    
  }

  navigateToBalanceByAccount(currentLevel = undefined) {
    if (!currentLevel) {
      var currentLevelIndex = this.displayType.findIndex(dt => dt.id === this.displayTypeSelected);
      var currentLevelId = 0;
      if (this.displayType[currentLevelIndex].isDetail) {
        var findIndex = this.displayType.findIndex(f => f.id === this.displayType[currentLevelIndex - 1].id - 2 && f.isDetail === false);
        currentLevelId = this.displayType[findIndex].id;
      }
      else
        currentLevelId = this.displayType.filter(f => f.level === this.displayType[currentLevelIndex].level)[0].id;
    }
    else {
      currentLevelId = this.displayType.filter(f => f.level === currentLevel)[0].id;
    }
  ...
  onDblClick(event) {
    ...
    if (this.clickedRowItem) {      
      ...
      if (upperLevelExist) {
        ...
        this.navigateToBalanceByAccount(currentLevel);
        this.showMessage(this.getText('Balance.LevelIsNotExists'), MessageType.Warning);
      }
    }
  }
  ...
  onFooterExportToExcel(header:any , footer:any) {
    ...
    header.cells.forEach((cell) => {
      ...
      {
        var findIndex = this.gridColumns.findIndex(f => f.groupName === cell.value);
        if (findIndex > 0) {
