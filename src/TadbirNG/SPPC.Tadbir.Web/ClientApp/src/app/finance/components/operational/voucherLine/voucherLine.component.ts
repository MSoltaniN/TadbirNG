import { Component, OnInit, Input, Renderer2, ViewChild, Output, EventEmitter, ChangeDetectorRef, NgZone, ElementRef } from '@angular/core';
...
export class VoucherLineComponent extends AutoGeneratedGridComponent implements OnInit {
  ...
  constructor(public toastrService: ToastrService, public translate: TranslateService, public dialogService: DialogService, public gridService: GridService,
    ...
    public settingService: SettingService, public reporingService: ReportingService, public ngZone: NgZone, public bStorageService: BrowserStorageService,public elem:ElementRef) {
  ...
  openEditorDialog(isNew: boolean) {
    ...
    this.dialogRef.content.instance.save.subscribe((viewModel) => {
      ...
      var promise = new Promise((resolve) => {
        var serviceUrl = isNew ? String.Format(VoucherApi.VoucherArticles, this.voucherId)
          : String.Format(VoucherApi.VoucherArticle, model.id);

        if (this.subjectMode == 1)
        {
            serviceUrl = isNew ? String.Format(VoucherApi.DraftVoucherArticles, this.voucherId)
              : String.Format(VoucherApi.DraftVoucherArticle, model.id);
        }

        this.saveHandler(model, isNew, this.voucherLineService, serviceUrl).then((success: ResultOption) => {          
          resolve();       
        })
  ...
  deleteModel(confirm: boolean) {
    if (confirm) {
      ...
      if (this.groupDelete) {
        ...
        if (this.subjectMode == 1)
          url = VoucherApi.AllDraftVoucherArticles;

        this.voucherLineService.groupDelete(url, rowsId).subscribe(res => {
          var data: any = res;
        ...
        if (this.subjectMode == 1)
          url = VoucherApi.DraftVoucherArticle;

        this.voucherLineService.delete(String.Format(url, this.deleteModelId)).subscribe(response => {
          this.deleteModelId = 0;
          this.showMessage(this.deleteMsg, MessageType.Info);
          if (this.rowData.data.length == 1 && this.pageIndex > 1)
            this.pageIndex = ((this.pageIndex - 1) * this.pageSize) - this.pageSize;
  ...
  reloadGrid(options?:ReloadOption) {
    ...
    this.voucherLineService.getAll(String.Format(url, this.voucherId), this.pageIndex, this.pageSize, this.sort, filter).subscribe((res) => {
  ...
  changeVoucherStatus(mode: string) {
    ...
    if (this.subjectMode == 1) {
      this.showMessage(this.getText('VoucherLine.ApproveActionNotPermitted'), MessageType.Warning);
      setTimeout(() => {
