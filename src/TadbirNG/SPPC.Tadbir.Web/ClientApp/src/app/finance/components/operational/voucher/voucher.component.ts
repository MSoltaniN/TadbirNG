import { Component, OnInit, Renderer2, ChangeDetectorRef, ViewChild, NgZone, HostListener, ElementRef } from '@angular/core';
...
import { Layout, Metadatas, Entities, MessageType } from '@sppc/shared/enum/metadata';
import { MetaDataService, ReportingService, BrowserStorageService, GridService, LookupService, ErrorHandlingService } from '@sppc/shared/services';
...
import { ViewName, VoucherPermissions, ItemBalancePermissions, DraftVoucherPermissions, ManageVouchersPermissions, ManageDraftVouchersPermissions, GlobalPermissions } from '@sppc/shared/security';
...
export class VoucherComponent extends AutoGeneratedGridComponent implements OnInit {
  ...
  constructor(public toastrService: ToastrService, public translate: TranslateService, public dialogService: DialogService, public gridService: GridService,
    ...
    public errorHandlingService: ErrorHandlingService,public elem:ElementRef) {
  ...
  addVoucherStatusFilter() {
    ...
    this.activeRoute.params.subscribe(params => {      
      switch (params['mode']) {
        case "committed": {  
          var statusFilter = this.voucherService.getStatusFilter("2", this.branchSelected == "1" ? this.BranchId.toString() : undefined,true);
        ...
        case "finalized": {
          var statusFilter = this.voucherService.getStatusFilter("3", this.branchSelected == "1" ? this.BranchId.toString() : undefined, true);
          this.quickFilter.push(statusFilter.filter[0]);
          this.voucherStatusTypeLabel = this.getText(statusFilter.key)
          this.voucherStatusTypeSelected = true;
          break;
        ...
        case "confirmed": {
          var statusFilter = this.voucherService.getStatusFilter("4", this.branchSelected == "1" ? this.BranchId.toString() : undefined, true);
          this.quickFilter.push(statusFilter.filter[0]);
          this.voucherStatusTypeLabel = this.getText(statusFilter.key)
          this.voucherStatusTypeSelected = true;
          break;
        ...
        case "approved": {
          var statusFilter = this.voucherService.getStatusFilter("5", this.branchSelected == "1" ? this.BranchId.toString() : undefined, true);
          this.quickFilter.push(statusFilter.filter[0]);
          this.voucherStatusTypeLabel = this.getText(statusFilter.key)
          this.voucherStatusTypeSelected = true;
          break;
  ...
  async getGlobalPermissions() {
      ...
      if (this.selectedSubjectType == "-1") {                    
        this.exportAccessed = this.isAccess(Entities.DraftVouchers, exportCode) && this.isAccess(Entities.Vouchers, exportCode);
        this.printAccessed = this.isAccess(Entities.DraftVouchers, printCode) && this.isAccess(Entities.Vouchers, printCode);
        this.filterAccessed = this.isAccess(Entities.DraftVouchers, filterCode) && this.isAccess(Entities.Vouchers, filterCode);
      }
  ...
  openEditorDialog(isNew: boolean) {
    ...
    if (!isNew)
      subjectType = this.editDataItem.subjectType;

    this.dialogRef = this.dialogService.open({
      title: subjectType == "1" ? this.getText('Voucher.DraftVoucherDetail') : this.getText('Voucher.VoucherDetail'),
  ...
  checkEditPermission() {    
    if (this.isDraftsOnlySelected() && !this.isAccess(Entities.DraftVoucher, DraftVoucherPermissions.View)) {
      this.showMessage(this.getText('App.AccessDenied'), MessageType.Warning);
      return false;
    }

    if (this.isNormalsOnlySelected() && !this.isAccess(Entities.Voucher, VoucherPermissions.View)) {
      this.showMessage(this.getText('App.AccessDenied'), MessageType.Warning);
      return false;
    }
    ...
    switch(this.selectedSubjectType) {
      case "-1":
        {      
          ...
          if (this.selectedRows.length == 1 && this.DataArray[0].subjectType == 1) {
    ...
    if (confirm) {
      if (this.groupOperation) {
        ...
        if (this.validateMultiTypeSelection()) {
          ...
          this.voucherService.groupDelete(VoucherApi.EnvironmentVouchers, rowsId).subscribe(res => {
            ...
            if (data && data.length > 0) {
              ...
              this.showMessage(this.getText('Voucher.VouchersDeleted'), MessageType.Info);
            }
        ...
        this.voucherService.delete(String.Format(VoucherApi.Voucher, this.deleteModelId)).subscribe(response => {
          ...
          this.showMessage(this.getText('Voucher.VoucherDeleted'), MessageType.Info);
  ...
  onCheckHandler() {
    if (this.validateVoucherSelection() && this.validateMultiTypeSelection()) {
      if (this.groupOperation) {
        ...
        this.voucherService.changeVouchersStatus(url, rowsId).subscribe(res => {
          var data: any = res;
          if (data.length == null || data.length == 0) {
            var message = this.getText('Voucher.OperationSuccessful')
            this.showMessage(message, MessageType.Succes);
        ...
        this.voucherService.changeVoucherStatus(String.Format(url, this.selectedRows[0])).subscribe(res => {
          this.reloadGrid();
          this.selectedRows = [];
          this.DataArray = [];
        }, (error => {
