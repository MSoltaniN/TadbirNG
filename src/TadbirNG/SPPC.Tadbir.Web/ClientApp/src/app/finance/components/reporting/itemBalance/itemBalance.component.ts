  ...
  implements OnInit,OnDestroy
{
  ...
  openingAsFirstVoucher: boolean = false;
  ...
  scopes = ["ItemBalanceComponent","AutoGeneratedGridComponent"];
  ...
  loadEntity() {
    this.settingService.getAll(LookupApi.TreeViews).subscribe((res) => {
      ...
      if (this.selectedEntityId == undefined) this.selectedEntityId = result[0];
  ...
  onchangeEntity() {
    ...
    this.displayTypeSelected = undefined;
    this.fillDisplayTypes();
  ...
  fillDisplayTypes() {
    var url = String.Format(
      ItemBalanceApi.ItemBalanceTypeLookup,
      this.selectedEntityId.key
    );
  ...
  showSelectForm() {
    var currentDisplay = this.displayType.filter(
  ...
  setApiUrl(accountId) {
    if (!accountId) {
      ...
      var bookItem = this.displayType.filter(
        (bt) => bt.id === this.displayTypeSelected
      )[0];
  ...
  onChangeFilterByRef(event) {
    if (
      !this.isAccess(Entities.ItemBalance, ItemBalancePermissions.FilterByRef)
    ) {
      setTimeout(() => {
  ...
  showFilterComponent() {
    ...
    var filterDialogModel = <AdvanceFilterComponent>(
      this.dialogRef.content.instance
    );
  ...
  showAccountBook(selectedAccount) {
    ...
    if (this.itemBalanceType == "1") {
      ...
      if (this.clickedRowItem) {
        ...
        this.dialogModel.selectedEntityId = this.selectedEntityId.key;
        this.dialogModel.voucherStatusSelected = this.voucherStatusSelected;
        this.dialogModel.branchScopeSelected = this.branchScopeSelected;
        this.dialogModel.selectedBranchSeparation =
          this.selectedBranchSeparation;
        this.dialogModel.isApplyBranchSeparation = this.isApplyBranchSeparation;

        this.dialogModel.selectedViewId = parseInt(this.selectedEntityId.key);
