...
import { BranchScopeResource, VoucherStatusResource } from "@sppc/finance/enum";
...
import { BalanceByAccountPermissions, ViewName } from "@sppc/shared/security";
import {
...
export class BalanceByAccountComponent
  ...
  implements OnInit, OnDestroy
{
  ...
  scopes = ["BalanceByAccountComponent","AutoGeneratedGridComponent"];
  ...
  constructor(
    ...
    if (this.shareDataService.sharingData) this.initializeDate = false;
  ...
  ngOnInit() {
    ...
    promise.then(() => {
      ...
      if (this.isTestBalanceParams) {
        ...
        this.accountService
          .getAccountById(balanceAccountParam.account.accountId)
          .subscribe((account) => {
  ...
  fillByDefaultValues() {
    if (this.chbAccount) this.getAccountItemLevels(true, ViewName.Account);

    if (this.chbCCenter) this.getAccountItemLevels(true, ViewName.CostCenter);

    if (this.chbDetailAccount)
      this.getAccountItemLevels(true, ViewName.DetailAccount);

    if (this.chbProject) this.getAccountItemLevels(true, ViewName.Project);
  }
  ...
  fillInputParameters(balanceAccountParam) {
    ...
    this.selectedBranchSeparation =
      balanceAccountParam.selectedBranchSeparation;
  ...
  onBeforeQuickReportSetting() {
    ...
    if (
      (this.param && this.parameters != this.param.items) ||
      this.paramChanged
    ) {
  ...
  sortingGridColumns() {
    ...
    switch (parseInt(this.selectedReportBy)) {
      case ViewName.Account: {
        fullCodeColumn = this.gridColumnsRow.find(
          (f) => f.name.toLowerCase() == "accountfullcode"
        );
        nameColumn = this.gridColumnsRow.find(
      ...
      case ViewName.DetailAccount: {
        fullCodeColumn = this.gridColumnsRow.find(
          (f) => f.name.toLowerCase() == "detailaccountfullcode"
        );
        nameColumn = this.gridColumnsRow.find(
          (f) => f.name.toLowerCase() == "detailaccountname"
        );
        break;
      }
      case ViewName.CostCenter: {
        fullCodeColumn = this.gridColumnsRow.find(
          (f) => f.name.toLowerCase() == "costcenterfullcode"
        );
        nameColumn = this.gridColumnsRow.find(
          (f) => f.name.toLowerCase() == "costcentername"
        );
        break;
      }
      case ViewName.Project: {
        fullCodeColumn = this.gridColumnsRow.find(
          (f) => f.name.toLowerCase() == "projectfullcode"
        );
        nameColumn = this.gridColumnsRow.find(
          (f) => f.name.toLowerCase() == "projectname"
        );
        break;
      }
      default:
    ...
    this.dialogRef.content.instance.cancel.subscribe((res) => {
      switch (viewId) {
        case ViewName.Account: {
          ...
          this.selectedAccountName = this.getText(
            "BalanceByAccount.AllAccount"
          );
          break;
        }
        case ViewName.DetailAccount: {
          this.selectedDetailAccount = undefined;
          this.selectedDetailAccountName = this.getText(
            "BalanceByAccount.AllDetailAccount"
          );
          break;
        }
        case ViewName.CostCenter: {
          this.selectedCCenter = undefined;
          this.selectedCCenterName = this.getText(
            "BalanceByAccount.AllCostCenter"
          );
          break;
        }
        case ViewName.Project: {
          this.selectedProject = undefined;
          this.selectedProjectName = this.getText(
            "BalanceByAccount.AllProject"
          );
          break;
        }
        default:
  ...
  getAccountItemLevels(isChecked: boolean, viewId: number) {
    if (isChecked) {
      this.gridService
        .getModels(String.Format(LookupApi.AccountBookLevels, viewId))
        .subscribe((res) => {
          switch (viewId) {
            case ViewName.Account: {
              ...
              if (!this.selectedAccountInit) {
                ...
                this.displayTypeName =
                  this.accountLevelList[this.selectedAccountLevel].title;
              }
              break;
            }
            case ViewName.DetailAccount: {
              ...
              this.selectedDetailAccountLevel =
                this.detailAccountLevelList[
                  this.detailAccountLevelList.length - 1
                ].level;
              this.selectedDetailAccountName = this.getText(
                "BalanceByAccount.AllDetailAccount"
              );
            ...
            case ViewName.CostCenter: {
              ...
              this.selectedCCenterLevel =
                this.cCenterLevelList[this.cCenterLevelList.length - 1].level;
              this.selectedCCenterName = this.getText(
                "BalanceByAccount.AllCostCenter"
              );
              break;
            ...
            case ViewName.Project: {
              ...
              this.selectedProjectLevel =
                this.projectLevelList[this.projectLevelList.length - 1].level;
              this.selectedProjectName = this.getText(
                "BalanceByAccount.AllProject"
              );
              break;
  ...
  getColumns(e: any) {
    this.orginalGridColumns = e;
    let columnList: Array<any> = [];
    Object.assign(columnList, this.orginalGridColumns);

    if (!this.selectedBranchSeparation) {
      this.gridColumnsRow = columnList.filter(
        (f) =>
          f.name != "BranchName" &&
      ...
      this.gridColumnsRow = columnList;
    }
  ...
  changeBranchSeparation() {
    if (
      ...
      if (!this.selectedBranchSeparation) {
        this.gridColumnsRow = this.gridColumnsRow.filter(
          (f) => f.name != "BranchName"
        );
      } else {
        var branchColumn = this.orginalGridColumns.find(
          (f) => f.name == "BranchName"
        );
  ...
  changeParam(clearBreadCrumb: boolean = true) {
    ...
    if (
      ...
      var relatedDisplayType = this.accountLevelList.filter(
        (d) => d.key === this.selectedAccountLevel
      )[0];
      if (
  ...
  onChangeFilterByRef(event) {
    if (
      !this.isAccess(
        Entities.BalanceByAccount,
        BalanceByAccountPermissions.FilterByRef
      )
    ) {
      setTimeout(() => {
  ...
  getReportData(callBreadCrumb: boolean = true) {
      ...
      case ViewName.Account.toString(): {
        if (
          ...
          errorMsg = String.Format(
            this.getText("BalanceByAccount.SelectLevelErrorMsg"),
    ...
    if (errorMsg) {
      ...
      this.param = {
        ...
        detailAccountId: this.selectedDetailAccount
          ? this.selectedDetailAccount.id
          : null,
  ...
  generateOptionNumber() {
    var options =
      (this.useClosingTempVoucher ? BalanceOptions.UseClosingTempVoucher : 0) |
      ...
      switch (parseInt(this.selectedReportBy)) {
        case ViewName.Account: {
          var url = String.Format(
            AccountApi.Account,
            this.clickedRowItem.accountId
          );
          this.accountService.getById(url).subscribe((account) => {
            this.prapareBreadCrumb(
              account,
              this.accountLevelList,
              ViewName.Account
            );
        ...
        case ViewName.DetailAccount: {
          var url = String.Format(
            ...
            this.clickedRowItem.detailAccountId
          );
        ...
        case ViewName.CostCenter: {
          var url = String.Format(
            CostCenterApi.CostCenter,
            this.clickedRowItem.costCenterId
          );
  ...
  prapareBreadCrumb(account, accountLevelList, viewId) {
    if (
      accountLevelList.findIndex((f) => f.level === account.level + 1) > 0 &&
      !this.isTestBalanceParams
    ) {
      if (viewId == ViewName.Account) {
        this.selectedAccountLevel = accountLevelList.filter(
          (f) => f.level === account.level + 1
        )[0].key;
        this.selectedAccount = account;
        this.selectedAccountName = account.name;
        this.breadCrumbAdded = this.updateBreadCrumb(
          accountLevelList,
          this.selectedAccountLevel,
          account
        );
      ...
      if (viewId == ViewName.DetailAccount) {
        this.selectedDetailAccountLevel = accountLevelList.filter(
          (f) => f.level === account.level + 1
        )[0].key;
      ...
      if (viewId == ViewName.CostCenter) {
        this.selectedCCenterLevel = accountLevelList.filter(
          (f) => f.level === account.level + 1
        )[0].key;
      ...
      if (viewId == ViewName.Project) {
        this.selectedProjectLevel = accountLevelList.filter(
          (f) => f.level === account.level + 1
        )[0].key;
  ...
  onChangeVoucherStatus() {
    ...
    var statusFilter = this.voucherService.getStatusFilter(
      this.selectedVoucherStatus,
      this.selectedBranchScope == "1" ? this.BranchId.toString() : undefined
    );
  ...
  showColumn(viewId: number) {
    ...
    if (this.orginalGridColumns.length) {
      var fullCodeColumn;
      var nameColumn;

      switch (viewId) {
        case ViewName.Account: {
          fullCodeColumn = this.orginalGridColumns.find(
            (f) => f.name.toLowerCase() == "accountfullcode"
          );
          nameColumn = this.orginalGridColumns.find(
            (f) => f.name.toLowerCase() == "accountname"
          );
          break;
        }
  ...
  hideColumn(viewId) {
    ...
    this.gridColumnsRow = this.gridColumnsRow.filter(
      (f) =>
        f.name.toLowerCase() != fullCodeColumn &&
        f.name.toLowerCase() != nameColumn
    );
