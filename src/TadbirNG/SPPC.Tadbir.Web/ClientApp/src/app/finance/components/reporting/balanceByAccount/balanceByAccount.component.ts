...
import { Account, CostCenter, DetailAccount, Project } from '@sppc/finance/models';
import { AccountInfo, AccountService, CostCenterInfo, CostCenterService, DetailAccountInfo, DetailAccountService, ProjectInfo, ProjectService, VoucherService } from '@sppc/finance/service';
import { AccountApi, BalanceByAccountApi, CostCenterApi, DetailAccountApi, ProjectApi, VoucherApi } from '@sppc/finance/service/api';
import { AutoGeneratedGridComponent, Filter, FilterExpression, FilterExpressionOperator, String } from '@sppc/shared/class';
...
import { BrowserStorageService, GridService, LookupService, MetaDataService, ReportingService } from '@sppc/shared/services';
...
export class BalanceByAccountComponent extends AutoGeneratedGridComponent implements OnInit, OnDestroy  {
  ...
  getColumns(e: any) {
    ...
    if (!this.selectedBranchSeparation) {      
      this.gridColumnsRow = columnList.filter(f => f.name != "BranchName" &&
  ...
  changeBranchSeparation() {
    if (this.isAccess(Entities.BalanceByAccount, BalanceByAccountPermissions.ViewByBranch)) {
      ...
      if (!this.selectedBranchSeparation) {
        ...
        var branchColumn = this.orginalGridColumns.find(f => f.name == "BranchName");
  ...
  changeParam(clearBreadCrumb: boolean = true) {
    ...
    if (clearBreadCrumb && this.breadCrumbList.length && this.selectedAccountLevel != undefined) {
  ...
  showColumn(viewId: number) {
    ...
    if (this.orginalGridColumns.length) {
      ...
      switch (viewId) {
        case ViewName.Account: {
          fullCodeColumn = this.orginalGridColumns.find(f => f.name.toLowerCase() == "accountfullcode");
          nameColumn = this.orginalGridColumns.find(f => f.name.toLowerCase() == "accountname");
          break;
        }
        case ViewName.DetailAccount: {
          fullCodeColumn = this.orginalGridColumns.find(f => f.name.toLowerCase() == "detailaccountfullcode");
          nameColumn = this.orginalGridColumns.find(f => f.name.toLowerCase() == "detailaccountname");
          break;
        }
        case ViewName.CostCenter: {
          fullCodeColumn = this.orginalGridColumns.find(f => f.name.toLowerCase() == "costcenterfullcode");
          nameColumn = this.orginalGridColumns.find(f => f.name.toLowerCase() == "costcentername");
          break;
        }
        case ViewName.Project: {
          fullCodeColumn = this.orginalGridColumns.find(f => f.name.toLowerCase() == "projectfullcode");
          nameColumn = this.orginalGridColumns.find(f => f.name.toLowerCase() == "projectname");
          break;
        }
        default:
      }
