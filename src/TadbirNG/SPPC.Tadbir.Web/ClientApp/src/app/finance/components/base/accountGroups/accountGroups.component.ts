...
import { AccountItemBrief, AccountGroup, Account } from '@sppc/finance/models';
import { AccountGroupsService, AccountGroupInfo, AccountFullDataInfo } from '@sppc/finance/service';
import { AccountGroupApi, AccountApi } from '@sppc/finance/service/api';
import { SettingService } from '@sppc/config/service';
import { String, Filter, AutoGeneratedGridComponent } from '@sppc/shared/class';
import { ViewTreeConfig, ViewTreeLevelConfig } from '@sppc/config/models';
import { ViewName, Permissions, AccountGroupPermissions } from '@sppc/shared/security';
...
export class AccountGroupsComponent extends AutoGeneratedGridComponent implements OnInit {
  ...
  constructor(public toastrService: ToastrService, public translate: TranslateService, public dialogService: DialogService, public gridService: GridService,
    public cdref: ChangeDetectorRef, public renderer: Renderer2, public metadata: MetaDataService, public accountGroupService: AccountGroupsService, public bStorageService: BrowserStorageService,
    public settingService: SettingService, public reporingService: ReportingService, public ngZone: NgZone,public elem:ElementRef) {
    ...
    if (dataItem.id == -1) {
      ...
      if (dataItem.code) {
        ...
        if (allLedgerNodes.length > 0) {
          ...
          var ledgerNodes = this.accountGroupService.getModels(String.Format(AccountApi.LedgerAccountsByGroupId, dataItem.id));
  ...
  getAccountGroupParent() {
    if (this.parentGroupId) {
      this.accountGroupService.getById(String.Format(AccountGroupApi.AccountGroup, this.parentGroupId)).subscribe(res => {
  ...
  addNew() {
    if (this.isAccess(Entities.AccountGroup, AccountGroupPermissions.Create)) {
      if (this.selectedItem.id == -1) {
        ...
        this.accountGroupService.getById(String.Format(AccountApi.NewChildAccount, this.parentId ? this.parentId : "0")).subscribe(res => {
  ...
  editHandler() {
    if (this.isAccess(Entities.AccountGroup, AccountGroupPermissions.Edit)) {
      ...
      this.grid.loading = true;
      var url = this.selectedItem.id == -1 ? AccountGroupApi.AccountGroup : AccountApi.AccountFullData;//AccountApi.Account;
  ...
  deleteModel(confirm: boolean) {    
    if (confirm) {
      ...
      if (this.groupOperation && !this.selectedContextmenu) {
        ...
        this.grid.loading = true;
        var url = this.selectedItem.id == -1 ? AccountGroupApi.AccountGroups : AccountApi.EnvironmentAccounts;
        ...
        this.accountGroupService.groupDelete(url, this.selectedModelsIdArray).subscribe(res => {
  ...
  contextMenuAddNewHandler() {
    ...
    if (this.isAccess(ViewName[this.viewId], new Permissions().getPermission(entityPermission, 'Create')))
      this.addNew();
    else
  ...
  contextMenuEditHandler() {
    ...
    if (this.isAccess(ViewName[this.viewId], new Permissions().getPermission(entityPermission, 'Edit'))) {

      this.grid.loading = true;
      
      var url = parentItem.id == -1 ? AccountGroupApi.AccountGroup : AccountApi.AccountFullData;
  ...
  contextMenuRemoveHandler() {
    var entityPermission = ViewName[this.viewId] + 'permissions';
    if (this.isAccess(ViewName[this.viewId], new Permissions().getPermission(entityPermission, 'Delete'))) {
      this.deleteConfirm = true;
  ...
  saveAccountHandler(model: any, isNew: boolean) {
    this.grid.loading = true;
    if (!isNew) {
      this.accountGroupService.edit<AccountFullData>(String.Format(AccountApi.Account, model.account.id), <AccountFullData>model)
        .subscribe(response => {
    ...
    if (model) {
      ...
      if (this.parentId) {
        ...
        if (this.selectedRows.length > 0) {
          ...
          var itemIndex = this.treeNodes.findIndex(f => f.id == this.selectedContextmenu.id && f.parentId == this.selectedContextmenu.parentId && f.groupId == this.selectedContextmenu.groupId);
          if (itemIndex > -1)
            this.treeNodes.splice(itemIndex, 1);
        }
    ...
    if (model) {
      ...
      if (this.groupOperation) {
        this.selectedModelsIdArray.forEach(id => {
          var accGroupIndex = this.treeNodes.findIndex(f => f.id == id && f.parentId == null && f.groupId == null);
          if (accGroupIndex > -1)
            this.treeNodes.splice(accGroupIndex, 1);
        })
      }
      else {
