...
export class CurrencyBookComponent extends AutoGeneratedGridComponent implements OnInit, OnDestroy {
  ...
  @Persist() selectedCurrencyValue: string = null;
  ...
  getCurrencies() {
    this.lookupService.GetCurrenciesLookup().subscribe(res => {
      this.currenciesRows.push({ key: "-1", value: this.getText('CurrencyBook.AllCurrencies') });
  ...
  getReportData() {
    ...
    if (parseInt(this.selectedCurrencyValue) > -1) {
