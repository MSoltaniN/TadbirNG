...
export class CashRegistersComponent extends AutoGeneratedGridComponent implements OnInit {
  ...
  async rolesHandler() {
    ...
    let selectedRoles = await lastValueFrom(this.cashRegisterService.getCashRegisterRoles(cashRegisterId));
  ...
  saveCashRegisterRoles(companyRoles: RelatedItems) {
    this.grid.loading = true;
    this.cashRegisterService.modifiedCashRegisterRoles(companyRoles).subscribe(
      (response) => {
        // this.roleList = false;
