...
export class AutoGeneratedGridComponent extends ListComponent implements OnDestroy {
  ...
  private reloadWithParam(insertedModel?: any, parameter?: any) {
    if (this.getDataUrl) {
      ...
      this.gridService.getAllByParams(this.getDataUrl, parameter,this.pageIndex, this.pageSize, this.sort, filter, filterExp, changed).subscribe((res) => {
    ...
    if (this.getDataUrl) {
      ...
      if (!this.parameters || this.parameters.length == 0) 
      {
        ...
        return this.gridService.getAllByParams(this.getDataUrl,param, 1, 1000000, this.sort, filter, filterExp, false).pipe(
          map(response => (<GridDataResult>{
            data: (response.body.comparativeItems) ? response.body.comparativeItems : response.body,
            total: (response.body.comparativeItems) ? response.body.comparativeItems.length : response.body.length,
          }))
        );
