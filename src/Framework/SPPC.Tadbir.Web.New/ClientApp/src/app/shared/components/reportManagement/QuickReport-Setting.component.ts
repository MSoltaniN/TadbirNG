...
export class QuickReportSettingComponent extends AutoGeneratedGridComponent  {
  ...
  showReportSetting(gridColumns: Property[], entityTypeName: string, viwId: number, rManager: ReportManagementComponent) {
    ...
    if (jsonString) {
      ...
      this.allGridColumn = new Array<any>();
      ...
      if (response) {
        ...
        this.reportManager.Grid.leafColumns.forEach((item) => {
          ...
          if (column.field) {
            ...
            if (this.allGridColumn.filter(a => a.name.toLowerCase() === qr.name.toLowerCase()).length == 0)
              this.allGridColumn.push(qr);
  ...
  getColumnRows(item) {
    return this.allGridColumn.filter(col => col.groupName === item.groupName);
  }
