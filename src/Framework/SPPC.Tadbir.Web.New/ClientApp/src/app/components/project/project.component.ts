...
export class ProjectComponent extends DefaultComponent implements OnInit {
  ...
  public saveHandler(model: Project, isNew: boolean) {
  ...
  */
  ...
  public reloadGrid(insertedModel?: Project) {
    if (this.viewAccess) {
      ...
      if (this.parentComponent != null && (this.goLastPage || (insertedModel && !this.addToContainer))) {
        ...
        this.projectService.getAll(ProjectApi.EnvironmentProjects, 0, 1, this.sort, filter).subscribe((res) => {
          if (res.headers != null) {
  ...
  loadGridData(insertedModel?: Project, filter?: FilterExpression) {

    this.projectService.getAll(ProjectApi.EnvironmentProjects, this.pageIndex, this.pageSize, this.sort, filter).subscribe((res) => {
