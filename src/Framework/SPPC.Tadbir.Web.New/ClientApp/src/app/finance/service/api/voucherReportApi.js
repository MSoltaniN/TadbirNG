// ------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool.
//     Runtime Version: 1.1.429
//     Template Version: 1.0
//     Generation Date: 10/20/2018 12:16:12 ب.ظ
//  
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
// ------------------------------------------------------------------------------
import { environment } from "../../../../environments/environment";
export class VoucherReportApi {
}
//reports/voucher/sum-by-date
VoucherReportApi.VoucherSumReport = environment.BaseUrl + "/reports/voucher/sum-by-date";
//reports/voucher/std-form/{voucherId:min(1)}
VoucherReportApi.VoucherStdFormReport = environment.BaseUrl + "/reports/voucher/std-form/{0}";
//# sourceMappingURL=voucherReportApi.js.map