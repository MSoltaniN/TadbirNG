...
import { ViewName, ItemBalancePermissions, AccountPermissions } from '@sppc/shared/security';
...
export class ItemBalanceComponent extends AutoGeneratedGridComponent implements OnInit {
  ...
  //#region Methods
  //fillDisplayTypes() {
  //  this.itemBalanceService.getAll(ItemBalanceApi.ItemBalanceTypeLookup).subscribe(res => {
  ...
  fillDisplayTypes() {
    var url = String.Format(ItemBalanceApi.ItemBalanceTypeLookup, this.viewId);
  ...
  getAccountBookTypes(isInitValue?: boolean) {
    this.gridService.getModels(String.Format(LookupApi.AccountBookLevels, this.selectedEntityId.key)).subscribe(res => {
      this.bookType = res;
      this.selectedBookType = res[0];
      //if (isInitValue) {
      //  this.initValue();
      //}
    })
  ...
  setApiUrl(accountId) {
    ...
    if (!accountId) {
      ...
      this.getDataUrl = String.Format(this.getDataUrl, this.viewId, bookItem.level);
    }
    else {
      switch (this.selectedEntityId.key) {
        case "6":
      ...
      this.viewId = ViewName[this.entityTypeName];
      //var bookItem = this.bookType.filter(bt => bt.key === this.selectedBookType.key)[0];
      this.getDataUrl = String.Format(this.getDataUrl, this.viewId, accountId);
    }
  ...
  showFilterComponent() {
    ...
    if (this.advanceFilterList) {
      ...
      //filterDialogModel.groupFilters = this.advanceGroupFilter;
  ...
  onDblClick(event) {
    if (this.clickedRowItem) {
      var currentLevel = this.bookType.filter(dt => dt.key === this.selectedBookType.key)[0].level
      var upperLevelExist = this.bookType.filter(dt=> dt.level > currentLevel).length > 0;
      if (upperLevelExist) {
        this.getReportData(this.clickedRowItem.accountId);
        this.changeDisplayTypeByAccId();
        this.updateBreadCrumb(this.clickedRowItem);
