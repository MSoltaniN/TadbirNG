...
export class VoucherComponent extends AutoGeneratedGridComponent implements OnInit {
  ...
  */
  ...
  UnGroupConfirmModel(confirm: boolean) {
    console.log('UnGroupConfirmModel');
    if (confirm) {
      this.grid.loading = true;
      let rowsId: Array<number> = [];
      this.selectedRows.forEach(item => {
      ...
      console.log('inside if___rowsid');
      console.log(rowsId);
  ...
  isDisabledConfirmGroupBtn = () => {
    if (this.selectedRows.length == 1 || this.selectedRows.length == 0) {
      ...
      for (var i = 0; i < this.selectedRows.length; i++) {
        var index = this.rowData.data.findIndex(rd => rd.id === this.selectedRows[i]);
        if (index >= 0 && this.rowData.data[index].statusId == DocumentStatusValue.NormalCheck) {
          if (this.rowData.data[index].approvedById == null && this.rowData.data[index].approverName == null) {
            conflit = true;
            break;
          }
          if (this.rowData.data[index].confirmedById == null && this.rowData.data[index].confirmerName == null) {
            conflit = true;
            break;
          }
        }
  ...
  onConfirmGroupHandler() {
    if (this.groupOperation) {
      ...
      console.log('درآینده نزدیک امکان تایید گروهی کامل می گردد');
    }
  }
  onUnConfirmGroupHandler() {
    console.log('onUnConfirmGroupHandler');
    ...
    //}
    //else {
    //  console.log('درآینده نزدیک امکان رفع تایید گروهی کامل می گردد');
    //}
