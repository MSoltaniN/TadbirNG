...
export class TestBalanceComponent extends AutoGeneratedGridComponent implements OnInit, OnDestroy {
  ...
  openSelectForm() {
    ...
    if (parseInt(this.displayTypeSelected) == BalanceDisplayType.BySubsidiaryOfLeader)
      this.dialogModel.defaultCriteria = new Filter("level", "0", " == {0}", "System.String")

    if (parseInt(this.displayTypeSelected) == BalanceDisplayType.ByDetailOfSubsidiary)
  ...
  changeType() {
    ...
    if (parseInt(this.displayTypeSelected) <= 2) {
    ...
    if (!accountId) {
      if (this.testBalanceType) {
        ...
        if (!accountId) {
          ...
          var displayTypeLevel = this.displayType.filter(f => f.id === parseInt(this.displayTypeSelected))[0].level;
