...
export class TestBalanceComponent extends AutoGeneratedGridComponent implements OnInit, OnDestroy {
  ...
  reloadGrid(insertedModel?: any) {
    if (this.getDataUrl) {
      ...
      var quickFilter: FilterExpression;
      this.quickFilter.forEach(item => {
        quickFilter = this.addFilterToFilterExpression(quickFilter,
          item, FilterExpressionOperator.And);
      })
      ...
      this.gridService.getAll(this.getDataUrl, this.pageIndex, this.pageSize, this.sort, filter, quickFilter).subscribe((res) => {
