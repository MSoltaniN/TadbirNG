...
export class TestBalanceComponent extends AutoGeneratedGridComponent implements OnInit {
  ...
  onFooterExportToExcel(header:any , footer:any) {
    ...
    header.cells.forEach((cell) => {
      ...
      else if (cell.value == this.gridColumns[3].groupName) {
        footer.cells[index].value = this.startBalanceCredit;
        footer.cells[index + 1].value = this.startBalanceDebit;
        index = index + 2;
      }
      else if (cell.value == this.gridColumns[5].groupName) {
        footer.cells[index].value = this.turnoverCredit;
        footer.cells[index + 1].value = this.turnoverDebit;
        index = index + 2;
      }
      else if (cell.value == this.gridColumns[7].groupName) {
        footer.cells[index].value = this.endBalanceCredit;
        footer.cells[index + 1].value = this.endBalanceDebit;
        index = index + 2;
      }
      else {
        index++;
